...
Full operator steps, the 21-gate table, behavior notes, known
limitations, virtio-win stream matrix, and guest-collector checklist:
...
CNV). Gate 11 queries Prometheus for 1h storage latency and reports
UNKNOWN if the query is skipped or returns no series -- also watch
`BSODRisk_StorageLatency*`.
