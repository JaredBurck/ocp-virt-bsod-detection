...
foreach ($v in $vectors) {
  ...
  # F-03/F-06: optional per-vector reason, asserted ONLY when present so the
  # long-standing vectors stay valid unchanged. The reason is what routes a
  # finding's DOMAIN (at_stream_max -> the weight-0 platform domain) and its
  # KCS/confidence (below_tooling_floor -> the 7128506 utility article), so a
  # verdict string that still matches while the reason silently changes is a
  # scoring regression the verdict assertion alone cannot see.
  $expectedReason = $v.expected_reason
  $actualReason = $script:StreamDriverVerdictReason

  $total++
  if ($expectedReason -and $actualReason -ne $expectedReason) {
    $failCount++
    $msg = "  FAIL: version=$($v.version) stream=$stream -> reason '$actualReason', expected '$expectedReason' ($note)"
    $failures += $msg
    Write-Host $msg
