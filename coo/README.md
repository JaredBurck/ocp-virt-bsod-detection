...
- Re-pointing the `alerts/` `PrometheusRule` CRs, the ACM policy's embedded copies, and every `promtool`/Prometheus-exec verification command in `alerts/README.md`, `windows-exporter/README.md`, and `docs/admin-runbook.md` at the new stack's API endpoint instead of `prometheus-k8s-0`/`thanos-querier.openshift-monitoring.svc`
...
| BSOD gate scripts | CLI tools, not cluster state or a console feature | `scripts/cnv-win-bsod-audit.sh` |
| Historical crash/QMP log archives | COO's troubleshooting panel (Part A) correlates *live* signals; it does not parse archived logs | Attach virt-launcher logs + gate `--json` to the support case |
