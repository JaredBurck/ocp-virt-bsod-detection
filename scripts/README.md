...
| `pg-must-gather-cmd.sh` | Bash | Generate `oc adm must-gather` command with all relevant images (CNV, MTV, BSOD) | `oc` |
...
| `ci/validate-doc-counts.py` | Python | Validate hardcoded gate/test counts in README/CLAUDE.md/agents.mdc/scripts/README.md match reality | `test-python` |
...
See "Exporting a Reviewable Artifact Instead of Mutating the Guest" in
`docs/operator-runbook.md` for the full `.reg`/Ansible-playbook coverage table.
...
- [`docs/operator-runbook.md`](../docs/operator-runbook.md) -- operational procedures and alert response guide
- [`tests/`](../tests/) -- lab validation VM fixtures and cross-layer verdict tests
- [`shared/virtio-win-thresholds.json`](../shared/virtio-win-thresholds.json) -- single source of truth for driver version thresholds
- [`insights-rules/`](../insights-rules/) -- Python analysis engine (broader rule set than the bash gates)
