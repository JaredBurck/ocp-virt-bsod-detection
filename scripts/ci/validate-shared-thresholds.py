...
def check_gate11_fallback_matches_recording_rule() -> None:
    """F-05: Gate 11's raw-counter fallback must track the recording rule.

    scripts/lib/prom-query.sh computes worst-direction latency from the raw
    KubeVirt counters when bsod:vmi_disk_latency:worst_1h is absent -- which is
    the normal case on a cluster that has not deployed the alerting CRs, and
    was the case on the validation cluster (5 live storage series, 0
    recording-rule series). That fallback is a SECOND copy of the recording
    rule's maths, so it can drift from it silently: the gate would keep
    returning a number, just a different one from what the alerts fire on.

    Binding them on the exact PromQL text would be too brittle to be useful
    (whitespace, label selectors, the {namespace,name} filter the gate adds).
    What matters, and what actually drifted in the F-01 defect this guards
    against, is the METRIC SET: the fallback must read both directions, exactly
    as the recording rule does. A fallback that quietly went read-only would
    reintroduce F-01 in the gate layer alone.
    """
    rules = (REPO_ROOT / "alerts" / "bsod-risk-recording-rules.yaml").read_text()
    lib = REPO_ROOT / "scripts" / "lib" / "prom-query.sh"
    if not lib.is_file():
        FAILURES.append("missing scripts/lib/prom-query.sh, which Gate 11 sources")
        return
    fallback = lib.read_text()
    ...
    for metric in sorted(required):
        if metric not in rules:
            FAILURES.append(
                f"alerts/bsod-risk-recording-rules.yaml no longer references "
                f"{metric} -- if the recording rule changed direction coverage, "
                f"Gate 11's fallback in scripts/lib/prom-query.sh must change "
                f"with it")
        if metric not in fallback:
            FAILURES.append(
                f"scripts/lib/prom-query.sh's Gate 11 fallback does not "
                f"reference {metric} -- it must compute the SAME "
                f"worst-direction quantity as bsod:vmi_disk_latency:worst_1h, "
                f"or the gate and the alerts disagree about the same VM")

    # The union idiom itself: a naive `(a >= b) or b` silently DROPS a disk
    # doing I/O in only one direction, which is the specific mistake the
    # recording rule's comment calls out.
    if "label_replace" not in fallback:
        FAILURES.append(
            "scripts/lib/prom-query.sh's fallback does not use the "
            "label_replace union idiom -- without it a one-direction disk is "
...
def check_agents_mdc_el10_fail_floor() -> None:
    """agents.mdc el10_1 FAIL < 1.9.x must match streams.el10_1.fail.

    L1: the cursor rule shipped FAIL < 1.9.52 after the JSON floor moved to
    1.9.53 (same class as check_stream_threshold_ordering). CLAUDE.md defers
    to the JSON and is left alone.
    """
    cfg = _load("virtio-win-thresholds.json")
    expected = ((cfg.get("streams") or {}).get("el10_1") or {}).get("fail")
    if not expected:
        FAILURES.append(
            "shared/virtio-win-thresholds.json: streams.el10_1.fail is missing")
        return
    agents = REPO_ROOT / ".cursor" / "rules" / "agents.mdc"
    # Cursor rules are GitLab-only. The public export prove step (and GitHub
    # Actions) run this validator on a tree that deliberately omits .cursor/.
    # Skip rather than fail closed on a file the allowlist does not ship.
    if not agents.is_file():
        return
    text = agents.read_text()
    m = re.search(r"el10_1:.*FAIL < (1\.9\.\d+)", text)
    if not m:
        FAILURES.append(
            ".cursor/rules/agents.mdc: no el10_1 'FAIL < 1.9.x' mention to bind")
        return
    if m.group(1) != expected:
        FAILURES.append(
            f".cursor/rules/agents.mdc el10_1 FAIL floor is {m.group(1)}, "
            f"shared/virtio-win-thresholds.json streams.el10_1.fail is {expected}")
...
def main() -> int:
    ...
    check_gate11_fallback_matches_recording_rule()
    ...
    check_agents_mdc_el10_fail_floor()
