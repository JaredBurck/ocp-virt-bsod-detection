...
# e.g. icacls DACL readback after /inheritance:r. Copied from
# cnv-qga-fleet-collect.sh (customer-shareable single-file constraint: do
# not add a shared-lib dependency).
qga_exec_capture() {
  local ns="$1" pod="$2" domain="$3" path="$4"
  shift 4
  local args_json="[]"
...
# Fail-closed DACL readback after icacls /inheritance:r.
...
qga_acl_readback_ok() {
  local ns="$1" pod="$2" domain="$3" path="$4"
  local acl
  acl=$(qga_exec_capture "$ns" "$pod" "$domain" "C:\\Windows\\System32\\icacls.exe" "$path" 2>/dev/null)
  if [ -z "$acl" ]; then
    echo "ERROR: could not read ACL on $path (icacls returned nothing)" >&2
    return 1
  fi
  # Any remaining Users/Everyone/Authenticated Users ACE means inheritance
  # removal did not take. Same regex as harden_guest_staging_dir in
  # cnv-qga-fleet-collect.sh.
  if printf '%s' "$acl" | grep -qiE 'S-1-5-32-545|S-1-1-0|S-1-5-11|\\Users:|Everyone:|Authenticated Users:'; then
    echo "ERROR: $path still grants access to non-administrative users after hardening:" >&2
    echo "$acl" >&2
    return 1
  fi
  return 0
...
for VM in $candidate_vms; do
  ...
  for _stage_dir in "C:\\ProgramData\\windows_exporter" "C:\\Temp\\windows-exporter" \
    ...
    # H3: fail closed. qga_exec only returns icacls's exit code (0 even when
    # Users still has an ACE). Abort this VM's staging -- do not run the
    # installer -- if the DACL is unreadable or still grants Users/Everyone.
    if ! qga_acl_readback_ok "$NAMESPACE" "$POD" "$DOMAIN" "$_stage_dir"; then
      red "  [ERROR] ACL readback failed on $_stage_dir -- aborting staging (will not run installer)"
      stage_ok=0
      break
    fi
  done
  ...
  if [ "$stage_ok" != "1" ]; then
    [ -n "$STATUS_FILE" ] && echo -e "$VM\tFAIL\tacl-readback-failed" >> "$STATUS_FILE"
    FAILED=$((FAILED + 1))
    continue
  fi
  ...
  for local_f in "${!STAGE_MAP[@]}"; do
    ...
    if ! qga_acl_readback_ok "$NAMESPACE" "$POD" "$DOMAIN" "$guest_f"; then
      red "  [ERROR] ACL readback failed on $guest_f -- aborting staging (will not run installer)"
      stage_ok=0
      break
    fi
  done
