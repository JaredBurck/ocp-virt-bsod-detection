...
function Get-StreamDriverVerdict {
  ...
  # F-03/F-06: reason channel, mirroring bash's DRIVER_VERDICT_REASON and
  # Python's details['verdict_reason']. The verdict STRING alone is ambiguous
  # -- several distinct conditions all return 'WARN' -- and two of them carry
  # real consequences downstream:
  #   at_stream_max       -> scored in the weight-0 `platform` domain, because
  #                          on a capped stream it is the only reachable
  #                          verdict and no driver update can clear it
  #   below_tooling_floor -> cited under KCS-7128506 (a collection utility
  #                          article) rather than KCS-7141291, the difference
  #                          between GENERAL-KNOWLEDGE and KCS-VALIDATED
  #
  # Deliberately a $script: variable rather than a richer return type: this
  # file is dot-sourced, and collect-windows-guest-info.ps1 plus
  # bsod-textfile-collector.ps1 both consume the bare [string] return. Changing
  # that signature would break them. Same globals-as-return-channel idiom bash
  # already uses here.
  $script:StreamDriverVerdictReason = ''
  ...
  if ($StreamMax -and $DriverVersion -ge $StreamMax) {
    $script:StreamDriverVerdictReason = 'at_stream_max'
    return 'WARN'
  }
  ...
  if ($DriverVersion -lt $ToolingFloor) {
    $script:StreamDriverVerdictReason = 'below_tooling_floor'
