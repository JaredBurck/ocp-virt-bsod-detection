...
evaluate_driver_version_stream() {
  ...
  if ! version_gte "$ver" "$TOOLING_FLOOR"; then
    # F-06 (v0.25.0 peer review follow-up): tag the reason so Gate 15 can cite
    # KCS-7128506 ("Windows guest data collection") rather than its default
    # KCS-7141291. That distinction is worth real score: 7128506 is a UTILITY
    # article, deliberately excluded from shared/risk-scoring.json's
    # kcs_trigger_articles because "how to collect data" is not "this causes a
    # BSOD". This branch says the driver is too old for the COLLECTION TOOLING
    # to work, which is a tooling-capability statement, not a crash trigger.
    #
    # Python has always cited 7128506 here and scored 3.15
    # (WARN 3 x driver 1.5 x GENERAL-KNOWLEDGE 0.7); bash emitted under Gate
    # 15's 7141291 and scored 4.50 (KCS-VALIDATED). Same finding, two numbers,
    # and the (severity, gate, kcs)-keyed contract vectors could never catch it
    # because they never see which kcs the audit script actually passes.
    # Reachable in production on el8_6 (OCP 4.12) and el9_2 (OCP 4.13-4.15) for
    # any version below the stream ceiling.
    DRIVER_VERDICT_REASON="below_tooling_floor"
