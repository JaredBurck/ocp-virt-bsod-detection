...
gate then reports UNASSESSED rather than a pass. Every other gate uses only
'get'/'list' and needs no elevated permission.
EOF
      exit 0
      ;;
    *)
...
_gate6_livemigrate_verdict() {
  ...
  if [ "${_vmi_present:-0}" != "1" ]; then
    ...
    #
    # (Emitting UNKNOWN here would also make a clean audit unreachable in EITHER
    # state, since Gate 11 emits UNKNOWN unconditionally for running VMs.)
    #
  ...
  for v in $vms; do
    ...
    if gate_enabled 11; then
      ...
      if [ -n "$vmi_exists" ]; then
        storage_classes="${_F[14]}"
        if [ -n "$storage_classes" ]; then
          info "  PVC/DataVolume references: $storage_classes"
          # The gate itself states it cannot measure latency -- that is an
          # absence of evidence, not a detected risk. UNKNOWN keeps it visible
          # without inflating the VM's risk tier as a confirmed finding.
          unknown "storage latency NOT measured -- the cluster API cannot read guest I/O latency. Verify via Prometheus BSODRisk_StorageLatency* / guest baselines / analyze.py. If spikes approach the ~60s VirtIO IoTimeoutValue, increase IoTimeoutValue (KCS-7132512)."
        else
