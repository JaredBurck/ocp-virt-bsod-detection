...
# F-06: BSOD_CLUSTER_CACHE_DIR is a PUBLIC contract, so it must be safe when
# a caller other than cnv-mtv-plan-gate.sh sets it.
#
# The shipped caller is safe by construction -- `mktemp -d` plus a trap, so the
# cache cannot outlive one gate run. But that is a property of ONE caller, not
# of this variable, and the variable is exported and advertised in CHANGELOG.md
# as the way "a fleet caller [can] warm those fetches once". An operator
# building their own wave loop who points it at a persistent directory (the
# obvious reading) previously got node labels, templates, instancetypes and the
# cluster VM list served from an arbitrarily old snapshot -- including after an
# `oc` context switch to a DIFFERENT cluster, where every cached answer is not
# merely stale but describes the wrong fleet entirely.
#
# The cache is therefore stamped with the identity of what produced it, and
# rejected when either component disagrees:
#
#   server -- read from the kubeconfig (`oc whoami --show-server`), a local
#             read with no apiserver round-trip. Catches the context switch,
#             which is the dangerous case: every cached answer then describes
#             the wrong fleet, not merely an older one.
#   age    -- catches a same-cluster cache that is simply too old to trust.
#             BSOD_CLUSTER_CACHE_TTL (default 900s) also bounds the window in
#             which a rebuilt-but-identically-addressed cluster can serve wrong
#             answers, which the server URL alone cannot detect.
#
# The QUERY is handled by the key rather than the stamp: the cache filename
# includes a checksum of the actual `oc` argument vector, so if a fetch's
# arguments ever change (a new label selector, a different resource) the key
# changes with them and the old file is simply never read. That closes the
# "same variable name, different question" drift without a tool-version string
# here -- which would have added a second thing to remember at release time,
# and this file has no TOOL_VERSION today. `cksum` is POSIX and always present;
# the key only has to separate a handful of query strings, not resist attack.
#
# Validation runs ONCE per process, not per fetch.
_CACHE_IDENTITY_CHECKED=0
_BSOD_CACHE_TTL="${BSOD_CLUSTER_CACHE_TTL:-900}"

_cache_identity_ok() {
  [ "$_CACHE_IDENTITY_CHECKED" -eq 1 ] && return 0
  _CACHE_IDENTITY_CHECKED=1
  local __dir="${BSOD_CLUSTER_CACHE_DIR:-}"
  [ -n "$__dir" ] || return 0
  # An unusable cache dir must disable caching, never abort the audit.
  mkdir -p "$__dir" 2>/dev/null || { BSOD_CLUSTER_CACHE_DIR=""; return 0; }

  local __server __stamp __want __have __age __now __stamped
  __server=$(timeout 10 oc whoami --show-server 2>/dev/null || echo "unknown")
  __want="server=${__server}"
  __stamp="$__dir/.bsod-cache-identity"
  __now=$(date +%s)

  if [ -f "$__stamp" ]; then
    __have=$(head -1 "$__stamp" 2>/dev/null)
    __stamped=$(sed -n '2p' "$__stamp" 2>/dev/null)
    case "$__stamped" in ''|*[!0-9]*) __stamped=0 ;; esac
    __age=$(( __now - __stamped ))
    if [ "$__have" = "$__want" ] && [ "$__age" -ge 0 ] && [ "$__age" -le "$_BSOD_CACHE_TTL" ]; then
      return 0   # same cluster, fresh enough
    fi
    # Purge ONLY the files this script owns. Never `rm -rf` the directory: a
    # caller may have pointed BSOD_CLUSTER_CACHE_DIR at a path holding other
    # data, and silently deleting that would be far worse than a stale cache.
    local __v
    for __v in $_OC_CACHEABLE; do rm -f "$__dir/$__v".*.json 2>/dev/null; done
    rm -f "$__stamp" 2>/dev/null
    if [ "$__have" = "$__want" ]; then
      info "  (cluster cache discarded: older than ${_BSOD_CACHE_TTL}s)"
    else
      info "  (cluster cache discarded: belongs to a different cluster)"
    fi
  fi
  printf '%s\n%s\n' "$__want" "$__now" > "$__stamp" 2>/dev/null || true
  return 0
}

# --- F-03: per-VM object index, O(1) per lookup -----------------------------
#
# CACHED_VM_INDEX/CACHED_VMI_INDEX were built once (correctly, and commented as
# an O(1) optimisation) but every per-VM read piped the WHOLE index through a
# fresh jq. So the intended O(1) lookup was O(N), up to four times per VM --
# O(N^2) overall, on the migration-blocking path, at exactly the 500-1000 VM
# scale R-23 was written for.
#
# Measured against the mock cluster with no network in the path: 50 -> 400 VMs
# took 6.1s -> 89.9s (8x the VMs, ~15x the time), with per-VM cost climbing
# from 0.121s to 0.225s. Isolating one lookup class: 400 single-key jq passes
# over the resulting 1.69 MB index cost 39.9s on their own.
#
# The index is now a directory of per-VM files built in ONE jq pass, so a
# lookup is a file read instead of a full-document parse. The split is
# lossless: JSON escapes tabs and newlines inside strings, so no VM object can
# contain a literal one, and `name<TAB>compact-json` per line round-trips
# exactly (verified against objects carrying tabs, quotes and newlines).
#
# The write loop uses printf redirection, a shell builtin -- no fork per VM,
# which would have given back most of the win.
# The root and its cleanup trap MUST be established in the main shell.
# _index_build is called inside a command substitution, and an EXIT trap set
# there fires when that SUBSHELL exits -- which would delete the index the
# instant it was built, leaving every VM reporting "cannot read VM". Creating
# it here, at top level, is what makes the trap cover the real process.
_INDEX_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/bsod-audit-idx.XXXXXX" 2>/dev/null) || _INDEX_ROOT=""
[ -n "$_INDEX_ROOT" ] && trap 'rm -rf "$_INDEX_ROOT"' EXIT

# _index_build <slot> <json-doc> -> echoes the index dir (empty on failure).
# One jq pass over the list, one file per item, keyed by metadata.name.
_index_build() {
  local __slot="$1" __doc="$2" __dir __name __body
  [ -n "$_INDEX_ROOT" ] || return 0
  __dir="$_INDEX_ROOT/$__slot"
  rm -rf "$__dir" 2>/dev/null
  mkdir -p "$__dir" 2>/dev/null || return 0
  while IFS=$'\t' read -r __name __body; do
    # k8s object names are DNS-1123 and cannot contain '/', so the name is a
    # safe leaf filename. Skip anything unexpected rather than trusting it.
    case "$__name" in ''|*/*|.|..) continue ;; esac
    printf '%s' "$__body" > "$__dir/$__name.json"
  done < <(printf '%s' "$__doc" | jq -r '.items[]? | "\(.metadata.name)\t\(tojson)"' 2>/dev/null)
  printf '%s' "$__dir"
}

# _index_get <index-dir> <name> -- echoes the object, or nothing if absent.
# Absence is indistinguishable from the old `.[$n] // empty`, which every
# caller already treats as "not present".
_index_get() {
  local __dir="$1" __name="$2"
  [ -n "$__dir" ] || return 0
  [ -f "$__dir/$__name.json" ] || return 0
  cat "$__dir/$__name.json"
}

_oc_cache_path() {  # _oc_cache_path <var-name> <oc-args...> -> path, or empty
  local __v="$1"; shift
  [ -n "${BSOD_CLUSTER_CACHE_DIR:-}" ] || return 0
  case "$_OC_CACHEABLE" in
    *" $__v "*) : ;;
    *) return 0 ;;
  esac
  [ -n "${BSOD_CLUSTER_CACHE_DIR:-}" ] || return 0
  local __key
  __key=$(printf '%s ' "$@" | cksum | tr -d ' ' )
  printf '%s/%s.%s.json' "$BSOD_CLUSTER_CACHE_DIR" "$__v" "$__key"
}
...
_oc_fetch() {   # _oc_fetch <var-name> <what> -- <oc args...>
  ...
  # Warm-cache hit: skip the API entirely. The key covers the `oc` arguments,
  # so a changed query can never be answered from a file written for the old
  # one (F-06).
  ...
  while :; do
    ...
    # F-11 (found live): LOCAL misconfiguration is permanent too. A kubeconfig
    # with no usable credentials makes `oc` fail instantly with "Missing or
    # incomplete configuration info", which matched none of the patterns above,
    # so every fetch burned its full retry budget on a condition no retry can
    # fix: 38s instead of 1s against a real cluster with a broken token. The
    # audit still reported correctly -- this is purely wasted wall-clock on the
    # migration-blocking path, and it only shows up with a real `oc` binary,
    # which is why the fixture harness never saw it.
    if printf '%s' "$__out" | grep -qiE 'forbidden|unauthorized|cannot list|not found|no matches for kind|invalid|bad request|missing or incomplete configuration|no configuration has been provided|must be logged in'; then
      break
    fi
...
if command -v oc >/dev/null 2>&1; then
  # F-06: validate/purge the cluster cache HERE, in the main shell, before any
  # fetch consults it.
  #
  # This was originally called from _oc_cache_path, which is always invoked as
  # `__cache=$(_oc_cache_path ...)` -- a command substitution that captures
  # stdout. The discard notice (an `info`, i.e. an echo) was therefore captured
  # INTO the cache path, producing a two-line $__cache and a broken redirect on
  # the next write. Found only by running against a real cluster: under the
  # mock, `oc whoami --show-server` always returned the same "unknown", so a
  # cold cache never disagreed with a warm one and the discard branch never ran
  # in the same process as a subsequent cache write.
  #
  # Once per process, in the main shell, is also simply the right place for it:
  # it is a whole-cache decision, not a per-key one.
  _cache_identity_ok
...
if [ "$PER_VM_ONLY" -eq 0 ]; then
  ...
  # F-07: acquired via _oc_fetch, like every other cluster read.
  #
  # These two fetches were the last raw `oc ... 2>/dev/null` reads left in the
  # cluster-scope path, and they sat TWELVE LINES above the ACQUISITION ERRORS
  # banner that exists to report exactly the failure they were discarding. All
  # three of _oc_fetch's guarantees were absent here:
  #
  #   - retry (R-24): a single 429 or apiserver rolling restart became a
  #     permanent UNKNOWN. Under --fail-on-unknown (the PreHook default, see
  #     tekton/bsod-gate-prehook-job.yaml) that BLOCKS a migration wave on a
  #     blip -- the precise scenario R-24 was written to prevent.
  #   - error capture (H3): `2>/dev/null` threw away the `oc` message, so the
  #     operator saw [UNKN] with no cause and no banner. Verified by forcing
  #     ONLY this fetch to fail: the gates reported UNKNOWN and the banner
  #     never printed, because nothing had appended to ACQUISITION_ERRORS.
  #   - caching (R-23): CACHED_SCOPE_VM_JSON is listed in _OC_CACHEABLE but
  #     never reached _oc_cache_path, so the entry was dead configuration --
  #     the cache file was never written OR read. Now genuinely wired for the
  #     -A path.
  #
  # The per-namespace loop deliberately fetches into _scope_doc, which is NOT
  # in _OC_CACHEABLE: one cache key cannot hold N namespaces' results without
  # them colliding. Only the cluster-wide -A form is cacheable, and it is the
  # only one whose result is identical for every per-VM invocation.
  #
  # _oc_fetch substitutes {"items":[]} on failure rather than leaving the var
  # empty, so the H3 contract below still holds: the _ERR flag -- not
  # emptiness -- is what distinguishes "could not read" from "genuinely none".
  CACHED_SCOPE_VM_JSON=""
  CACHED_SCOPE_VM_JSON_ERR=0
  if [ "$ALL_NAMESPACES" -eq 1 ]; then
    _oc_fetch CACHED_SCOPE_VM_JSON "VMs across all namespaces" -- get vm -A \
      || CACHED_SCOPE_VM_JSON_ERR=1
  else
    _scope_vm_docs=""
    for _scope_ns in "${NAMESPACES[@]}"; do
      _scope_doc=""
      _oc_fetch _scope_doc "VMs in namespace $_scope_ns (cluster-scope)" -- get vm -n "$_scope_ns" \
        || CACHED_SCOPE_VM_JSON_ERR=1
...
for current_ns in "${NAMESPACES[@]}"; do
  ...
  # Pre-index VMs by name for genuinely O(1) lookups (F-03). One jq pass
  # writes one file per VM; each per-VM read below is then a file read rather
  # than a full re-parse of the whole namespace.
  CACHED_VM_INDEX_DIR=$(_index_build "vm" "$CACHED_VM_LIST_JSON")
  ...
  # CACHED_VMI_INDEX_DIR below must treat a missing entry as ambiguous (could be a
  ...
  CACHED_VMI_INDEX_DIR=$(_index_build "vmi" "$CACHED_VMI_LIST_JSON")
  ...
  for v in $vms; do
    ...
    spec=$(_index_get "$CACHED_VM_INDEX_DIR" "$v")
    ...
    if [ -n "$vm_it_name" ] || [ -n "$vm_pref_name" ]; then
      if [ "$SPEC_EXPANSION" = "false" ]; then
        ...
        vmi_spec=$(_index_get "$CACHED_VMI_INDEX_DIR" "$v")
    ...
    if gate_enabled 6; then
      ...
      #
      # The condition is read from CACHED_VMI_INDEX_DIR, which already holds the
      # full VMI object -- no additional API call.
      _lm_status=""; _lm_reason=""; _vmi_present=0
      if [ -n "${CACHED_VMI_INDEX_DIR:-}" ]; then
        IFS=$'\t' read -r _vmi_present _lm_status _lm_reason < <(
          _index_get "$CACHED_VMI_INDEX_DIR" "$v" | jq -r '
            . as $vmi
            | if $vmi == null then ["0","",""]
              else ["1",
        ...
        # An absent VMI yields no output at all, so the reader above leaves the
        # fields empty -- normalise to the same "0" the null branch produced.
        [ -z "$_vmi_present" ] && _vmi_present=0
      fi
    ...
    IFS=$'\t' read -r _vmi_found _vmi_guest_os < <(
      _index_get "$CACHED_VMI_INDEX_DIR" "$v" | jq -r '
        . as $vmi | if $vmi then ["yes", ($vmi.status.guestOSInfo.id // "")] | @tsv
        else ["no", ""] | @tsv end' 2>/dev/null)
    # No file -> no output -> empty; that is the old "no" branch.
    [ -z "$_vmi_found" ] && _vmi_found="no"
    vmi_exists=""
      ...
      if [ -n "$vmi_exists" ]; then
        ...
        if [ -n "$pvc_refs" ]; then
          ...
          if [ -n "$_lat" ]; then
            ...
            unknown "storage latency NOT measured (${PQ_UNAVAILABLE_REASON:-no matching Prometheus series for this VM}) -- absence of data is not evidence of health. Verify via Prometheus BSODRisk_StorageLatency* or guest I/O baselines. If spikes approach the ~60s VirtIO IoTimeoutValue, increase IoTimeoutValue (KCS-7132512)."
          fi
        else
...
if [ "$JSON_MODE" = "" ]; then
  ...
  echo " NOTE: This gate covers cluster-visible checks plus guest files you"
  echo " collected under BSOD_GUEST_EVIDENCE_DIR. Exit 0 is not migration-safe."
  echo " Attach --json output and collector artifacts to the support case."
