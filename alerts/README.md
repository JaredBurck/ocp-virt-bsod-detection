...
1. **Hypervisor alerts** -- 12 alerting rules based on KubeVirt metrics (hypervisor-side)
...
Arch-capabilities (KCS-7125237) is not a Prometheus alert; use Gate 9 / `analyze.py` instead.
...
> (it never fires) but **do not present it to customers as a working detection mechanism
> today**. Until the metric ships, use virt-launcher QMP log analysis via must-gather
> instead (see `insights-rules/plugins/bsod_crashdump_checks.py`).
...
See `docs/operator-runbook.md`'s "`os` Label Dependency" section for the full pre-flight check and remediation (patching the `vm.kubevirt.io/os` annotation).
...
- **`evictionStrategy=None` not detectable from Prometheus**: The `evictable` label reports `true` for both `LiveMigrateIfPossible` and `None`. Use the BSOD audit script (Gate 6) or Python analyzer.
...
| `BSODRisk_GuestCrash` never fires | Expected on every shipping OCP Virt version -- the underlying metric has not shipped yet. Use must-gather QMP log analysis instead. |
